...
public class InventoryGrid
{
    ...
    public void RemoveItem(ItemData item)
    {
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    grid[x, y].item = null;
                    grid[x, y].itemStartX = -1;
                    grid[x, y].itemStartY = -1;
                }
            }
        }
    }

    public bool MoveItem(ItemData item, int fromX, int fromY, int toX, int toY)
    {
        RemoveItem(item);
        if (PlaceItem(item, toX, toY))
        {
            return true;
        }
        else
        {
            PlaceItem(item, fromX, fromY);
            return false;
        }
    }
    ...
    // Метод для получения всех предметов для сохранения.
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                if (grid[x, y].item != null && !savedItems.Contains(grid[x, y].item))
                {
                    ...
                    {
                        itemName = grid[x, y].item.itemName,
