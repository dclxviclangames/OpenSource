...
using System.Collections;
using ExitGames.Client.Photon; // Добавлено для EventData
using UnityEngine.SceneManagement;
...
public class GameManager : MonoBehaviourPunCallbacks //IOnEventCallback
{
    ...
    // Пользовательский код события для смерти игрока. Должен совпадать с PlayerHealth.cs.
    private const byte PlayerDiedEventCode = 100;

    public Transform scoreboardContainer; // Ссылка на пустой объект на канвасе с Vertical Layout Group
    public GameObject scoreTextPrefab; // Ссылка на префаб TextMeshProUGUI, который вы только что создали
    private Dictionary<Photon.Realtime.Player, TMPro.TextMeshProUGUI> playerScoresUI = new Dictionary<Photon.Realtime.Player, TMPro.TextMeshProUGUI>();
    public static string PlayerScoreKey = "playerScore";

   // public UnityEvent<Vector3> OnWallDestroyed = new UnityEvent<Vector3>();
    ...
    void Start()
    {
     ...
     //   OnWallDestroyed.AddListener(HandleWallDestroyed);
    }

    private void HandleWallDestroyed(Vector3 position)
    {
        // ВАШ КОД, который вызывается при уничтожении стены
        Debug.Log($"GameManager: Запись хода! Уничтожено здание на позиции {position}.");
    }
    ...
    [PunRPC]
    void RPC_GameOver(string outcome)
    {
        Debug.Log($"[GameManager] Игра окончена. Исход: {outcome}");

        // Здесь можно показать финальный UI.

        // Безопасный выход с задержкой
        StartCoroutine(LeaveRoomWithDelay(3.0f));
    }

    IEnumerator LeaveRoomWithDelay(float delay)
    {
        // Ждем 3 секунды, чтобы все сетевые события и UI успели отработать.
        yield return new WaitForSeconds(delay);

        // 1. **Проверка состояния** - предотвращает ошибку Operation LeaveRoom (254)
        if (PhotonNetwork.InRoom)
        {
    ...
    public override void OnPlayerLeftRoom(Player otherPlayer)
    {
        ...
        if (spawnedPlayers.ContainsKey(otherPlayer) && spawnedPlayers[otherPlayer] != null)
        {
            //PhotonNetwork.Destroy(otherPlayer);
            spawnedPlayers.Remove(otherPlayer);
        }

        if (playerScoresUI.ContainsKey(otherPlayer))
        {
            Destroy(playerScoresUI[otherPlayer].gameObject);
            playerScoresUI.Remove(otherPlayer);
        }
    }
    ...
    public override void OnPlayerPropertiesUpdate(Player targetPlayer, Hashtable changedProps)
    {
        ...
        if (changedProps.ContainsKey(PlayerScoreKey))
        {
            // Находим нужный UI-элемент в словаре
            if (playerScoresUI.ContainsKey(targetPlayer))
            {
                UpdatePlayerScoreUI(targetPlayer, playerScoresUI[targetPlayer]);
            }
        }

        //NEW LOGIC END ROUND НЕ СЛОМАЙ ИГРУ!!! 03.10.2025
    ...
    //check ALIVE PLAYERS
    private void CheckIfTeamWipedOut()
    {
        // Получаем список всех игроков в комнате
        Dictionary<int, Player> allPlayers = PhotonNetwork.CurrentRoom.Players;

        // Проверяем, есть ли среди них ХОТЯ БЫ ОДИН ЖИВОЙ
        // Мы используем LINQ для чистоты кода:
        bool anyPlayerStillAlive = allPlayers.Values.Any(player =>
        {
            // 1. Проверяем, содержит ли игрок свойство "IsAlive"
            if (player.CustomProperties.TryGetValue("IsAlive", out object statusValue))
            {
                // 2. Возвращаем true, если статус == true (игрок жив)
                return (bool)statusValue == true;
            }

            // Если свойство еще не установлено, считаем его живым (по умолчанию)
            return true;
        });

        // Если ни один игрок не считается живым
        if (!anyPlayerStillAlive)
        {
            Debug.Log("ВСЕ ИГРОКИ МЕРТВЫ. ПОРАЖЕНИЕ.");

            // Завершаем раунд. (false = поражение команде)
            MenuComback();
           // EndRound(false);
        }
    }

    private void UpdatePlayerScoreUI(Photon.Realtime.Player player, TMPro.TextMeshProUGUI scoreText)
    {
        // Получаем имя игрока и его счёт из Custom Properties
        string playerName = player.NickName;
        int playerScore = 0;
        scoreText.text = $"{playerName}: {playerScore}";
        if (player.CustomProperties.ContainsKey(PlayerScoreKey))
        {
            playerScore = (int)player.CustomProperties[PlayerScoreKey];
        }
    ...
    private void CreatePlayerScoreUI(Player player)
    {
        if (playerScoresUI.ContainsKey(player)) return;

        // Создаем новый UI-элемент из префаба
         
        GameObject scoreUIObject = Instantiate(scoreTextPrefab, scoreboardContainer);
        TMPro.TextMeshProUGUI scoreText = scoreUIObject.GetComponent<TMPro.TextMeshProUGUI>();

        // Добавляем в словарь
        playerScoresUI.Add(player, scoreText);

        // Изначально обновляем UI, используя текущие свойства
        UpdatePlayerScoreUI(player, scoreText);
    }
    ...
    private void SpawnPlayerFor(Player photonPlayer)
    {
        ...
        if(!playerScoresUI.ContainsKey(photonPlayer))
            CreatePlayerScoreUI(photonPlayer);
    ...
    private GameObject TryInstantiatePlayerPrefab(Player photonPlayer)
    {
        ...
        {
            string prefabName = selectedCharData.characterPrefab.name;
          //  photonPlayer.NickName = "Player " + selectedCharData.characterPrefab.name + ": ";
    ...
    private void ConfigurePlayerComponents(Player photonPlayer, GameObject playerGameObject)
    {
        ...
        if (photonPlayer.IsLocal)
        {
            ...
            // MainChanging Gravity Mechanic add multiplayer
            PlayerMovement playerMovement = playerGameObject.GetComponent<PlayerMovement>();
            // PlayersController playersController = playerGameObject.GetComponent<PlayersController>();
    ...
    private IEnumerator WaitForPlayerCustomProperties(Player player, string propertyKey, System.Action callback)
    {
        float timeout = 10f; // Увеличьте таймаут до 10 секунд (по желанию)
        float startTime = Time.time;
        ...
        if (player.CustomProperties.ContainsKey(propertyKey))
        {
            // Свойство пришло вовремя, спавним!
            callback?.Invoke();
        }
        else
        {
            Debug.LogWarning($"Timeout waiting for property {propertyKey} for player {player.NickName}. Player will be spawned when properties are fully updated.");
            // !!! УДАЛИТЕ ВЫЗОВ callback?.Invoke(); !!!
            // callback?.Invoke(); // <--- ЭТОТ ВЫЗОВ НАДО УДАЛИТЬ
        }
    }
